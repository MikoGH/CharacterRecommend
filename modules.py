...
def count_cosine(df: pd.DataFrame, character):
    top = 20
    df_similarity = pd.DataFrame(columns=['#', 'Name', 'Anime', 'Similarity'], index=df.index)
    for i, elm in df.iterrows():
        df_similarity.loc[i, 'Name'] = df.loc[i,'Name']
        df_similarity.loc[i, 'Anime'] = df.loc[i,'Anime']
        # columns = pd.concat([pd.Series(df.loc[i, 'Tags']), pd.Series(character['Tags'])]).unique()
        # df_similarity.loc[i, 'Similarity'] = round(1 - cosine(df.loc[i, columns], character[columns]),4)
    
        df_similarity.loc[i, 'Similarity'] = round(1 - cosine(df.loc[i, (df.columns != 'Name') & (df.columns != 'Anime')].values[1:], character[(df.columns != 'Name') & (df.columns != 'Anime')].values[1:]),4)
